...
- **Mark (final, ago 2026):** a hexagonal cell holding three members. It is the same shape every member avatar wears and the same idea the product is about: a few people, contained. It replaces the Font Awesome `bugs` glyph, which was always a placeholder. Source of truth is `app/public/assets/brand/hive-mark.svg`: one path with `fill-rule="evenodd"`, so the ring, the field and the dots come out of a single silhouette and recolouring is one attribute.
