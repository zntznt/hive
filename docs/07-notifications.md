...
WhatsApp is where the user's groups already live, so it is the **primary** channel. Email exists so WhatsApp is never a single point of contact (and because some invitees may only have an email). Web push is deliberately absent: WhatsApp *is* the push channel for these groups.
